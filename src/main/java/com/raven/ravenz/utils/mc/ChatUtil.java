...
import com.raven.ravenz.utils.other.StringUtils;
import com.raven.ravenz.utils.render.ColorUtils;
...
public final class ChatUtil implements IMinecraft {
    ...
    public static void warningChatMessage(final Text message) {
        if (Objects.isNull(mc.inGameHud)) {
            RavenZClient.INSTANCE.getLogger().warn(message.getString());
            return;
        }
    ...
    public static void errorChatMessage(final Text message) {
        if (Objects.isNull(mc.inGameHud)) {
            RavenZClient.INSTANCE.getLogger().error(message.getString());
            return;
        }
    ...
    public static void chatMessage(final MutableText message, final boolean prefix) {
        if (Objects.isNull(mc.inGameHud)) {
            RavenZClient.INSTANCE.getLogger().info(message.getString());
            return;
        }
