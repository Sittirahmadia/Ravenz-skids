...
public record BuiltOutline(
    ...
    public void render(Matrix4f matrix, float x, float y, float z) {
        ...
        float border = Math.max(0.5f, thickness);
        border = Math.min(border, Math.min(width, height) * 0.5f);
        ...
        addQuad(builder, matrix, x, y, z, width, border, color.color1(), color.color1(), color.color4(), color.color4());
        addQuad(builder, matrix, x, y + height - border, z, width, border, color.color2(), color.color2(), color.color3(), color.color3());
        addQuad(builder, matrix, x, y + border, z, border, Math.max(0.0f, height - border * 2.0f), color.color1(), color.color2(), color.color2(), color.color1());
        addQuad(builder, matrix, x + width - border, y + border, z, border, Math.max(0.0f, height - border * 2.0f), color.color4(), color.color3(), color.color3(), color.color4());
