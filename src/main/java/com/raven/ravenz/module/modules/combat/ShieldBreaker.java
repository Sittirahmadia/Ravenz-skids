...
public final class ShieldBreaker extends Module {
    ...
    private void onTickEvent(HandleInputEvent event) {
        ...
        if (attackTimer.hasElapsedTime(attackDelay.getValueInt()) || savedSlot == -1) {
            ...
            // autoStun: second attack on next tick via attackTimer, not same tick (avoids AC flag)
            // We set a short attackTimer so second hit fires quickly on next event
            if (autoStun.getValue() && attackDelay.getValueInt() <= 0) {
            ...
            attackTimer.reset();
            swapBackTimer.reset();
            // Keep breakingShield = true until we swap back
        }
    }
