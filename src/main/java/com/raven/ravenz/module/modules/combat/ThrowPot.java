...
public final class ThrowPot extends Module {
    private final KeybindSetting throwKey = new KeybindSetting("Throw Key", GLFW.GLFW_KEY_G, false);
    private final NumberSetting throwDelay = new NumberSetting("Throw Delay", 50, 1000, 250, 50);
    private final NumberSetting healthThreshold = new NumberSetting("Health Threshold", 1, 20, 10, 0.5);
    private final BooleanSetting multiThrow = new BooleanSetting("Multi Throw", true);
    ...
    private void throwNextPotion() {
        if (potionSlots.isEmpty()) { finishThrow(); return; }
        int index = potsThrown % potionSlots.size();
        int slot = potionSlots.get(index);
        // Re-validate the slot still has a health potion
        if (!isHealthPotion(mc.player.getInventory().getStack(slot))) {
            findPotionSlots();  // re-scan
            if (potionSlots.isEmpty()) { finishThrow(); return; }
            slot = potionSlots.get(0);
        }
