...
public class AnchorMacroV2 extends Module {
    ...
    private final NumberSetting itemSwap    = new NumberSetting("Item Swap",    1,  9, 1, 1);
    ...
    public AnchorMacroV2() {
        super("AnchorMacroV2", "Press Mouse 5 once to charge and explode a respawn anchor", -1, Category.COMBAT);
    ...
    private void onTick(TickEvent event) {
        ...
        // Rising-edge detection — fires only on the tick the button is first pressed
        boolean mouse5Now = GLFW.glfwGetMouseButton(
                mc.getWindow().getHandle(), GLFW.GLFW_MOUSE_BUTTON_5) == GLFW.GLFW_PRESS;
        ...
        // Cooldown gate after a successful explosion
        if (hasAnchored) {
        ...
        // ── Uncharged anchor → charge with glowstone ─────────────────────────
        if (charges == 0) {
        ...
        // ── Charged anchor → swap to target slot and explode ─────────────────
