...
public final class AutoRefill extends Module {
    ...
    private final NumberSetting hoverDelay = new NumberSetting("Hover Delay", 10, 100, 25, 5);
    private final NumberSetting minStack = new NumberSetting("Min Stack", 1, 64, 16, 1);
    private final BooleanSetting hoverMode = new BooleanSetting("Hover Mode", false);
    private final BooleanSetting health = new BooleanSetting("Health", true);
    private final BooleanSetting regen = new BooleanSetting("Regen", true);
    private final BooleanSetting strength = new BooleanSetting("Strength", false);
    private final BooleanSetting speed = new BooleanSetting("Speed", false);
    private final BooleanSetting tntCarts = new BooleanSetting("TNT Carts", false);
    ...
    private boolean needsRefill() {
        for (int i = 0; i < 9; i++) {
            ...
            // Only refill if it's a potion slot that's running low - not empty non-potion slots
            if (isPotion(stack) && stack.getCount() < minStack.getValueInt()) {
                return true;
            }
        }
        return false;
